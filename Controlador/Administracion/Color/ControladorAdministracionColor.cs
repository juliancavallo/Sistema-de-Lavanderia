...
{
    public class ControladorAdministracionColor
    {
        ...
        #region Formulario
        public ControladorAdministracionColor(Form frmAbm, Form pFrmAlta)
        {
            formAlta = pFrmAlta;
        ...
        private void CargarGrilla(List<Color> source)
        {
            DataGridView dgv = (DataGridView)controles.Find(x => x.Name == "gridColores");
            if (dgv != null)
            {
                dgv.DataSource = null;
        ...
        private void ClickBaja(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    colorBLL.Baja(colorBLL.Obtener(int.Parse(id)));
                    CargarGrilla(colorBLL.ObtenerTodos());
                }
            }
            catch (Exception ex)
        ...
        private void ClickBuscar(object sender, EventArgs e)
        {
            try
            {
                ...
                CargarGrilla(colorBLL.ObtenerTodos(descripcion));
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
