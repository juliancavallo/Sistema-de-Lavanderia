...
{
    public class ControladorAdministracionCategoria
    {
        ...
        #region Formulario
        public ControladorAdministracionCategoria(Form frmAbm, Form pFrmAlta)
        {
            formAlta = pFrmAlta;
        ...
        private void CargarGrilla(List<Categoria> source)
        {
            DataGridView dgv = (DataGridView)controles.Find(x => x.Name == "gridCategorias");
            if (dgv != null)
            {
                dgv.DataSource = null;
        ...
        {
            CargarGrilla(CategoriaBLL.ObtenerTodos());
        }
        ...
        private void ClickAlta(object sender, EventArgs e)
        {
            try
            {
                formAlta.Text = "Alta de Categoria";
        ...
        private void ClickBaja(object sender, EventArgs e)
        {
            try
            {
                var id = ServicioConfiguracionDeControles.ObtenerCampoSeleccionado(controles, "Id", "gridCategorias").ToString();
                ...
                {
                    CategoriaBLL.Baja(CategoriaBLL.Obtener(int.Parse(id)));
                    CargarGrilla(CategoriaBLL.ObtenerTodos());
                }
            }
            catch (Exception ex)
        ...
        private void ClickModificacion(object sender, EventArgs e)
        {
            try
            {
                ...
                formAlta.Controls.Find("IdCategoria", false).FirstOrDefault().Text = id;

                formAlta.Text = "Modificación de Categoria";
                formAlta.ShowDialog();
        ...
        private void ClickBuscar(object sender, EventArgs e)
        {
            try
            {
                ...
                CargarGrilla(CategoriaBLL.ObtenerTodos(descripcion));
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
