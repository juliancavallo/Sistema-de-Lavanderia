...
{
    public class ControladorRecepcionEnClinica
    {
        ...
        #region Formulario
        public ControladorRecepcionEnClinica(Form pForm)
        {
            ...
            try
            {
                if (DatosValidos(1))
                {
                    if (articuloBLL.ValidarBultosCompuestos((DataGridView)controles.Find(x => x.Name == "gridItems")))
                    {
                        ...
                        MessageBox.Show("La Recepcion fue creada exitosamente. Puede consultarla en Reportes > Recepciones en Clinica", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        frm.Hide();
                    }
                    else
